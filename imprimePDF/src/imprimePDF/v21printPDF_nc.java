	...
	//	String formato_factura = ".\\data\\20536659189\\10_formatos\\formatoSFS.jpg"; // .gif and .jpg are ok too!
      ...
      //  PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream(reportePDF));
		...
		document.add(img);
			...
			canvas.saveState();                               // q
			canvas.beginText();                               // BT
			canvas.moveText(466, 820);                         // 36 788 Td
			canvas.setFontAndSize(bf, 8); // /F1 12 Tf
			canvas.showText("Version xml UBL 2.1");	        // (Hello World)Tj
			canvas.endText();                                 // ET
	        ...
	        canvas.restoreState();                            // Q
			  ...
			  //	    Cabecera.set_total(0);
			  	     ...
			  	     for (int i=0; i<_lineas_de_la_factura; i++) {
			  	    	...
			  	    	//	double _neto_con_igv_valor=(Detalle[i].get_subtotal()-Detalle[i].get_descuento())+Detalle[i].get_igv();
			  	    		
			  	    		
			  	    		double _neto_linea=Detalle[i].get_cantidad()*Detalle[i].get_precio_unitario()*1.18;
			  	    		System.out.println("Linea "+i+"Cantidad: "+Detalle[i].get_cantidad()+" Precio:  "+Detalle[i].get_precio_unitario()+" Neto:"+_neto_linea);
			  //	    		double _neto_linea=Detalle[i].get_cantidad()*Detalle[i].get_precio_unitario()*1.18;
			  	    		
			  	    		Chunk _neto_con_igv = new Chunk(Formato.dinero(_neto_linea));
